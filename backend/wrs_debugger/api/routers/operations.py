...
async def manager(request: Request) -> OperationManager:
...
async def get_active_operation(request: Request) -> ActiveOperationResponse:
    return ActiveOperationResponse(operation=(await manager(request)).active())
...
async def get_operation(operation_id: str, request: Request) -> Operation:
    return (await manager(request)).get(operation_id)
