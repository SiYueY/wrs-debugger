...
from wrs_debugger.models.operation import Operation, OperationState, OperationType

Publish = Callable[[EventName, dict[str, object]], Awaitable[None]]
Work = Callable[[Callable[[str, float], Awaitable[None]]], Awaitable[None]]
...
class OperationManager:
    ...
    async def _run(self, operation: Operation, work: Work) -> None:
        ...
        await self._publish_operation(operation)
        try:
            # Let clients observe a newly accepted operation before a fast mock completes it.
            ...
            operation.error = "Operation interrupted during backend shutdown."
            raise
            ...
            operation.state = OperationState.failed
            operation.error = "INTERNAL_ERROR"
        else:
    ...
    async def _update(self, operation: Operation, stage: str, progress: float) -> None:
