...
const DeviceIdentity& Client::identity() const noexcept {
    return identity_;
}
...
hardware::Result<Bytes, Error> Client::exchange(
    ...
    for (std::uint8_t attempt = 0; attempt < max_attempts_; ++attempt) {
        ...
        if (response_transaction_id(response, expected_command) != transaction_id)
            continue;
...
hardware::Result<SdoFrame, Error> Client::read_sdo(SdoObject object) noexcept {
    if (!is_open())
        return Result<SdoFrame>::failure(Error::NotOpen);
    SdoFrame request{};
...
hardware::Result<void, Error> Client::write_sdo(const SdoFrame& request_frame) noexcept {
    if (!is_open())
        return Result<void>::failure(Error::NotOpen);
    ...
    const auto status = static_cast<std::uint16_t>(frame.object_index >> 12);
    if (!is_zero(frame.reserved.data(), frame.reserved.size()) ||
        (frame.object_index & 0x0fff) != static_cast<std::uint16_t>(SdoObject::UpgradeRequest))
...
hardware::Result<LoRaParamFrame, Error> Client::read_lora_parameters() noexcept {
    if (!is_open())
        return Result<LoRaParamFrame>::failure(Error::NotOpen);
    ...
    auto response = exchange(request.to_bytes(), request.transaction_id, SystemCmd::ParamReadRsp);
    if (!response)
        return Result<LoRaParamFrame>::failure(response.error());
    const auto frame = LoRaParamFrame::from_bytes(response.value());
    if (frame.object_index != 0x4000 || frame.object_data != 0 ||
        ParamFlags::from_raw(frame.param_flags).radio_type != RadioType::LoRa)
...
hardware::Result<GfskParamFrame, Error> Client::read_gfsk_parameters() noexcept {
    if (!is_open())
        return Result<GfskParamFrame>::failure(Error::NotOpen);
    ...
    auto response = exchange(request.to_bytes(), request.transaction_id, SystemCmd::ParamReadRsp);
    if (!response)
        return Result<GfskParamFrame>::failure(response.error());
    const auto frame = GfskParamFrame::from_bytes(response.value());
    if (frame.object_index != 0x4000 || frame.object_data != 0 ||
        ParamFlags::from_raw(frame.param_flags).radio_type != RadioType::GFSK)
...
hardware::Result<void, Error> Client::write_lora_parameters(
    ...
    auto flags = ParamFlags::from_raw(request.param_flags);
    flags.radio_type = RadioType::LoRa;
    request.param_flags = flags.to_raw();
    request.cmd = static_cast<std::uint8_t>(SystemCmd::ParamWriteReq);
    request.transaction_id = next_transaction();
...
hardware::Result<PinFrame, Error> Client::read_pin() noexcept {
    if (!is_open())
        return Result<PinFrame>::failure(Error::NotOpen);
    PinFrame request{};
    ...
    if (!response)
        return Result<PinFrame>::failure(response.error());
